...
- 🎯 **학습 경로 탐색**: 단순 문장이 아닌, 선수강 요구사항의 '의미론적 분석(Semantic Match)'을 통한 교과목 자동 매핑
...
- **그래프 DB**: Neo4j 5.x
- **임베딩 모델**: Sentence-BERT 다국어 모델 (sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2)
- **언어**: Python 3.8+
- **주요 라이브러리**: 
  - neo4j-driver
...
1. 교과목 데이터 로드
2. 임베딩 생성 (교과목 이름 + 교과목개요 기반)
3. 전체 교과목 대상 코사인 유사도 분석 (SIMILAR_TO)
4. 선수강과목 매핑: 임베딩 70% + TF-IDF 30% 혼합 하이브리드 필터링 적용 (REQUIRES, Threshold 0.6)
5. 학수번호 기반 관계 추론 (IDENTICAL_ID)
6. 샘플 분석 실행
...
# 2. 데이터 로드 및 임베딩 생성
df = builder.load_course_data('course_all_aggregated.csv')
embeddings = builder.create_embeddings(df)  # 교과목 이름 + 교과목개요 기반

# 3. 유사도 및 관계 데이터 계산
edges = builder.compute_similarity(df, embeddings, threshold=0.5)
...
python run_course_graph.py --password your_password \
  build --csv-path course_all_aggregated.csv --threshold 0.5
...
// 선수강과목(Prerequisite) 의미론적 매핑 결과 검색
MATCH (c1:Course)-[r:REQUIRES]->(c2:Course)
RETURN c1.name AS source, c1.department AS dept, c2.name AS target, r.raw_text, r.similarity
ORDER BY r.similarity DESC
LIMIT 10;
```
